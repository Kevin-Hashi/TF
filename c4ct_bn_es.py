...
def plot_result(history):
    ...
    plt.legend(loc='best')
    plt.title('accuracy')
    fig_name = os.path.splitext(os.path.basename(__file__))[
        0] + '_accuracy.png'
...
def create_model_three_layers(num_classes, input_shape):
    ...
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.Conv2D(256, 3, padding='same', kernel_initializer='he_uniform',
                               kernel_regularizer=tf.keras.regularizers.l2(1e-4), activation='relu')(x)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.Conv2D(256, 3, padding='same', kernel_initializer='he_uniform',
                               kernel_regularizer=tf.keras.regularizers.l2(1e-4), activation='relu')(x)
    ...
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.Dense(num_classes, kernel_regularizer=tf.keras.regularizers.l2(
        1e-4), activation='softmax')(x)
...
def _main(num_data, input_shape, num_classes, epochs, batch_size):
    ...
    # モデル保存
    model_dir = 'c4ct_bn_es/'
    if os.path.exists(model_dir) == False:
        os.mkdir(model_dir)
    num = 3
    name = os.path.splitext(os.path.basename(__file__))[0]+f'_model_{num}_{batch_size}.h5'
